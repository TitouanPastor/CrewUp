...
async def get_optional_current_user(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[dict]:
    """
    Extract user information from token if provided, otherwise return None.

    Use this for endpoints that have optional authentication.

    Usage:
        @router.get("/public")
        async def get_public(current_user: Optional[dict] = Depends(get_optional_current_user)):
            if current_user:
                # Authenticated user
                pass
            else:
                ...
                pass
    """
    # In test mode, return None (unauthenticated)
    if TESTING:
        return None
    ...
    try:
        ...
        logger.debug(f"Optional auth failed (acceptable): {e}")
        return None
